#!/usr/bin/env python3
"""Install mentor-score-loop into global and team-scoped Codex homes."""
...
def _agents_from_team(team_dir: Path) -> list[str]:
    ...
    if not cfg.exists():
        cfg = team_dir / "claudeteam.cloud.toml"
...
def install_team(team_dir: Path, state_dir: Path | None = None) -> list[Path]:
    ...
    for agent in _agents_from_team(team_dir):
        installed.append(_copy_skill(state / "codex-home" / agent / "skills"))
    return installed
