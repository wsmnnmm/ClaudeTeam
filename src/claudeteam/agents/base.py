...
# Braille-pattern spinner glyphs that Ink/Rich/Bubbletea-style CLIs commonly
# use for "I'm busy" indication. Concrete adapters splice this into their own
# busy_markers() return.
SPINNER_CHARS = ("⣾", "⣽", "⣻", "⢿", "⡿", "⣟", "⣯", "⣷")
...
class CliAdapter(ABC):
    ...
    def busy_markers(self) -> list[str]:
        """If any string appears at the pane tail, the agent is busy.

        Marker-free pane state is preferred for newer flows, but older
        adapters/tests still use this hook.
        """
        return []

    def rate_limit_markers(self) -> list[str]:
        """Strings that mean the CLI is currently rate-limited."""
        return []
