...
_COMMANDS: dict[str, str] = {
    ...
    "say": (
        ...
        "\n"
        "**R168 default: every `claudeteam say` now sends a v2 card** with a\n"
        "colored header (manager → blue, worker_* → green) so the group reads\n"
        "as structured updates, not raw chat text. Just write your status /\n"
        "report / 报道 normally:\n"
        "\n"
        "    claudeteam say <your-name> \"【你-name 报道】当前状态：在线 ✅\"\n"
        "\n"
        "For one-line acknowledgements (`收到`, `开始处理`) use `--no-card` to\n"
        "drop back to plain text:\n"
        "\n"
        "    claudeteam say <your-name> \"收到\" --no-card\n"
        "\n"
        "Cards don't thread (`--reply <id>` is silently ignored on the card\n"
        "path); use `--no-card --reply om_xxx` if threading matters.\n"
    ),
    "task": (
