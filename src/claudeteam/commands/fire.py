...
def main(argv: list[str]) -> int:
    ...
    has_window = tmux.has_window(target)
    if not cfg and not has_window and not paths.agent_dir(agent).exists():
        if archive.find_archived(agent) or local_facts.is_retired(agent):
            print(f"⚠️  {agent} has no live pane in session {session}")
            print(f"  ⏭  nothing to archive for {agent} (already fired?)")
            print(f"  ⚠️ roster removal skipped: unknown agent: {agent}")
