"""Tests for feishu/lark.py — subprocess wrapper around lark-cli."""
from __future__ import annotations

import contextlib
import os
import subprocess
from dataclasses import dataclass

from claudeteam.feishu import lark


@dataclass
class _R:
    returncode: int = 0
    stdout: str = ""
    stderr: str = ""


class _Recorder:
    def __init__(self, result=_R()):
        self.calls: list[dict] = []
        self.result = result

    def __call__(self, args, **kwargs):
        self.calls.append({"args": list(args), "kwargs": dict(kwargs)})
        return self.result


@contextlib.contextmanager
def _no_proxy_env():
    old = os.environ.get("LARK_CLI_NO_PROXY")
    os.environ["LARK_CLI_NO_PROXY"] = "1"
    os.environ["HTTPS_PROXY"] = "http://proxy.example:7890"
    try:
        yield
    finally:
        if old is None:
            os.environ.pop("LARK_CLI_NO_PROXY", None)
        else:
            os.environ["LARK_CLI_NO_PROXY"] = old
        os.environ.pop("HTTPS_PROXY", None)


def test_run_builds_npx_lark_cli_argv_with_profile():
    rec = _Recorder(_R(stdout='{"ok": true, "data": {"x": 1}}'))
    out = lark.run(["im", "+messages-send"], profile="my-team", run=rec)
    assert out == {"x": 1}
    sent = rec.calls[0]["args"]
    assert sent[:2] == ["npx", "@larksuite/cli"]
    assert "--profile" in sent and "my-team" in sent
    assert sent[-2:] == ["im", "+messages-send"]


def test_run_omits_profile_when_empty():
    rec = _Recorder(_R(stdout='{"data":{}}'))
    lark.run(["foo"], profile="", run=rec)
    sent = rec.calls[0]["args"]
    assert "--profile" not in sent


def test_run_returns_data_field_unwrapped():
    rec = _Recorder(_R(stdout='{"ok":true,"data":{"message_id":"om_1"}}'))
    assert lark.run(["x"], run=rec) == {"message_id": "om_1"}


def test_run_returns_full_object_when_no_data_field():
    rec = _Recorder(_R(stdout='{"raw":"value"}'))
    assert lark.run(["x"], run=rec) == {"raw": "value"}


def test_run_returns_empty_dict_for_blank_stdout():
    rec = _Recorder(_R(stdout=""))
    assert lark.run(["x"], run=rec) == {}


def test_run_returns_none_on_nonzero_exit():
    rec = _Recorder(_R(returncode=1, stderr="oops"))
    assert lark.run(["x"], run=rec) is None


def test_run_returns_none_on_invalid_json():
    rec = _Recorder(_R(stdout="not-json"))
    assert lark.run(["x"], run=rec) is None


def test_run_returns_none_on_timeout():
    def fake(*args, **kwargs):
        raise subprocess.TimeoutExpired(cmd=["lark"], timeout=90)
    assert lark.run(["x"], run=fake) is None


def test_run_default_timeout_is_90_seconds():
    rec = _Recorder(_R(stdout="{}"))
    lark.run(["x"], run=rec)
    assert rec.calls[0]["kwargs"]["timeout"] == 90


def test_run_uses_explicit_timeout_when_passed():
    rec = _Recorder(_R(stdout="{}"))
    lark.run(["x"], timeout=5, run=rec)
    assert rec.calls[0]["kwargs"]["timeout"] == 5


def test_run_strips_https_proxy_when_no_proxy_env_set():
    rec = _Recorder(_R(stdout="{}"))
    with _no_proxy_env():
        lark.run(["x"], run=rec)
    env = rec.calls[0]["kwargs"]["env"]
    assert "HTTPS_PROXY" not in env
    assert env.get("LARK_CLI_NO_PROXY") == "1"


def test_run_keeps_proxy_env_when_no_proxy_unset():
    rec = _Recorder(_R(stdout="{}"))
    os.environ["HTTPS_PROXY"] = "http://x"
    try:
        lark.run(["x"], run=rec)
        assert rec.calls[0]["kwargs"]["env"].get("HTTPS_PROXY") == "http://x"
    finally:
        os.environ.pop("HTTPS_PROXY", None)
